...
static bool vers_reset_alter_copy(THD *thd, TABLE *table)
{
  ...
  while (!(error= info.read_record()))
  {
    ...
    if (unlikely(error) && table->file->is_fatal_error(error, HA_CHECK_ALL))
    {
  ...
  if (will_batch)
  {
    if (unlikely(error= table->file->exec_bulk_update(&dup_key_found)))
      table->file->print_error(error, MYF(ME_FATALERROR));
...
err:
  if (unlikely(table->file->ha_external_lock(thd, F_UNLCK)))
    return true;
