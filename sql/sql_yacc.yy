...
part_type_def:
        ...
        | LIST_SYM 
	  {
        ...
        | LIST_SYM part_column_list
          { Lex->part_info->part_type= LIST_PARTITION; }
        | SYSTEM_TIME_SYM
          { if (unlikely(Lex->part_info->vers_init_info(thd))) MYSQL_YYABORT; }
        ...
        | ident '.' ident '(' opt_expr_list ')'
          {
            ...
            */
            ...
            if (unlikely(!$1.str || check_db_name((LEX_STRING*) &$1)))
              my_yyabort_error((ER_WRONG_DB_NAME, MYF(0), $1.str));
            if (unlikely(check_routine_name(&$3)))
            {
              MYSQL_YYABORT;
            }
        ...
        | CREATE TABLE_SYM table_ident opt_for_system_time_clause
          {
            ...
            if (unlikely(lex->vers_conditions.type != SYSTEM_TIME_UNSPECIFIED &&
...
opt_format_json:
        ...
        | FORMAT_SYM '=' ident_or_text
          {
            if (likely(!my_strcasecmp(system_charset_info, $3.str, "JSON")))
...
simple_ident_q2:
          '.' ident '.' ident
          {
            if (unlikely(!($$= Lex->create_item_ident(thd, &null_clex_str, &$2,
                           &$4))))
              MYSQL_YYABORT;
          }
        | ident '.' ident '.' ident
        ...
        | IDENT_QUOTED
          {
            ...
            {
              LEX_STRING to;
              if (unlikely(thd->convert_with_error(system_charset_info, &to,
                                                   thd->charset(), $1.str,
                                                   $1.length)))
                MYSQL_YYABORT;
              $$.str=    to.str;
...
ident_with_tok_start:
        ...
        | keyword
          {
            if (unlikely(!($$.str= thd->strmake($1.str, $1.length))))
              MYSQL_YYABORT;
            $$.length= $1.length;
            $$.m_pos= (char *) YYLIP->get_tok_start();
          }
...
query_expression_option:
        ...
        | HIGH_PRIORITY
          {
            if (unlikely(check_simple_select()))
              MYSQL_YYABORT;
