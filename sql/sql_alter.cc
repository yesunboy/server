...
*/
static const char* get_alter_algorithm(THD* thd)
{
  switch(thd->variables.alter_algorithm) {
  case DEFAULT_ALGO:
    return "ALTER_ALGORITHM=DEFAULT";
  case COPY_ALGO:
    return "ALTER_ALGORITHM=COPY";
  case INPLACE_ALGO:
    return "ALTER_ALGORITHM=INPLACE";
  case NOCOPY_ALGO:
    return "ALTER_ALGORITHM=NOCOPY";
  case INSTANT_ALGO:
    return "ALTER_ALGORITHM=INSTANT";
  }
  return NULL;
}
                                    ...
                                    const Alter_inplace_info &ha_alter_info)
{
  ...
  case HA_ALTER_INPLACE_COPY_LOCK:
    ...
    if (requested_algorithm == Alter_info::ALTER_TABLE_ALGORITHM_DEFAULT &&
        thd->variables.alter_algorithm >= NOCOPY_ALGO)
    {
      ha_alter_info.report_unsupported_error(get_alter_algorithm(thd),
                                             "ALGORITHM=INPLACE");
      return true;
    }
    return false;
  ...
  case HA_ALTER_INPLACE_NOCOPY_LOCK:
    ...
    if (requested_algorithm == Alter_info::ALTER_TABLE_ALGORITHM_DEFAULT &&
        thd->variables.alter_algorithm == INSTANT_ALGO)
    {
      ha_alter_info.report_unsupported_error("ALTER_ALGORITHM=INSTANT",
                                             "ALGORITHM=NOCOPY");
      return true;
    }
    return false;
  case HA_ALTER_INPLACE_NOT_SUPPORTED:
    ...
    if (requested_algorithm == Alter_info::ALTER_TABLE_ALGORITHM_DEFAULT &&
        thd->variables.alter_algorithm >= INPLACE_ALGO)
    {
      ha_alter_info.report_unsupported_error(get_alter_algorithm(thd),
                                             "ALGORITHM=COPY");
      return true;
